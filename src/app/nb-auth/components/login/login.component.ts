...
@Component({
  ...
  template: `
    <nb-auth-block>
      ...
      <small class="form-text sub-title">Witaj! Wprowadź swój email i hasło</small>

      <form (ngSubmit)="login()" #form="ngForm" autocomplete="nope">

        <div *ngIf="showMessages.error && errors && errors.length > 0 && !submitted"
             class="alert alert-danger" role="alert">
          <div><strong>O Kurczę!</strong></div>
          <div *ngFor="let error of errors">{{ error }}</div>
        </div>

        <div *ngIf="showMessages.success && messages && messages.length > 0 && !submitted"
             class="alert alert-success" role="alert">
          <div><strong>Udało Ci się zalogować!</strong></div>
        ...
        <div class="form-group accept-group col-sm-12">
          <nb-checkbox name="rememberMe" [(ngModel)]="user.rememberMe">Pamiętaj mnie</nb-checkbox>
      ...
      <div class="links">
        ...
        <small class="form-text">
          Nie masz konta <a routerLink="../register"><strong>Zarejestruj się</strong></a>
        </small>
      </div>
    </nb-auth-block>
  `,
